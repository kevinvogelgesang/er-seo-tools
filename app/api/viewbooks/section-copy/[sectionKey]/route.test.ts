...
vi.mock('@/lib/viewbook/template-service', () => ({
  putSectionCopyGlobalBridged: vi.fn(async () => {}),
  deleteSectionCopyGlobalBridged: vi.fn(async () => {}),
}))
...
describe('PUT /api/viewbooks/section-copy/[sectionKey]', () => {
  it('validates + writes and returns ok', async () => {
    ...
    expect(putSectionCopyGlobalBridged).toHaveBeenCalledWith('brand', { purpose: 'p', whatThis: 't', whatWeNeed: null }, 'op@er.com')
  })
...
describe('DELETE', () => {
  it('reverts to default', async () => {
    ...
    expect(res.status).toBe(200)
    expect(deleteSectionCopyGlobalBridged).toHaveBeenCalledWith('brand')
  })
})
