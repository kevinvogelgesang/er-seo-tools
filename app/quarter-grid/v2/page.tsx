...
const STORAGE_KEY = "seo-quarter-v2"
...
export default function QuarterGridV2() {
  ...
  const [prioritySet, setPrioritySet] = useState<Set<number>>(new Set())
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (d.prioritySet)  setPrioritySet(new Set(d.prioritySet))
      }
    } catch {}
    setLoaded(true)
  }, [])
  ...
  const persist = (overrides = {}) => {
    const state = {
      ...
      slotsPerWeek, snapshots, startDate,
      prioritySet: Array.from(prioritySet), ...overrides
    }
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(state)) } catch {}
  }

  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => { if (loaded) persist() }, [clients, schedule, completed, slotsPerWeek, snapshots, startDate, prioritySet])
  ...
  // Keyboard priority shortcut: hover chip in pool + press 1–5
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      const p = parseInt(e.key)
      if (p >= 1 && p <= 5 && hoveredId.current !== null) {
        const id = hoveredId.current
        setPriority(id, p)
        setPrioritySet(prev => { const n = new Set(prev); n.add(id); return n })
        flash(`⌨ ${getClient(id)?.name} → P${p}`)
      }
    }
  ...
  const unassignedKeyed = unassigned.filter(c => prioritySet.has(c.id))
    .sort((a, b) => a.priority - b.priority || a.name.localeCompare(b.name))
  const unassignedRaw = unassigned.filter(c => !prioritySet.has(c.id))
  ...
  const Chip = ({ id, fromWeek }: { id: number; fromWeek: number | null }) => {
    ...
    const isPoolChip = fromWeek == null
    return (
      <div
        draggable
        onDragStart={e => onDragStart(e, id, fromWeek)}
        onDragEnd={onDragEnd}
        onMouseEnter={() => { if (isPoolChip) hoveredId.current = id }}
        onMouseLeave={() => { if (isPoolChip) hoveredId.current = null }}
        className="chip-row"
        style={{
  ...
  return (
    <div style={{ fontFamily: "'DM Mono','Courier New',monospace", background: "#0f172a", minHeight: "100vh", padding: 16, color: "#e2e8f0" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", alignItems: "flex-start", gap: 12, justifyContent: "space-between" }}>
          <div style={{ minWidth: 200 }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 6 }}>
              <span style={{ fontSize: 15, fontWeight: 500, color: "#f1f5f9", letterSpacing: "-0.3px" }}>
                SEO Quarter Grid{' '}
                <span style={{ fontSize: 10, color: "#6366f1", background: "#1e1b4b", padding: "1px 6px", borderRadius: 4, marginLeft: 4 }}>V2</span>
              </span>
      ...
      {/* Priority Set section */}
      {unassignedKeyed.length > 0 && (
        <div
          style={{ marginBottom: 8, background: "#1e293b", border: "1px solid #334155", borderRadius: 10, overflow: "hidden" }}
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 14px", background: "#0f172a", borderBottom: "1px solid #334155" }}>
            <span style={{ fontSize: 10, fontWeight: 500, color: "#a78bfa", letterSpacing: "0.08em", textTransform: "uppercase" }}>⌨ Priority Set</span>
            <span style={{ fontSize: 10, fontWeight: 700, padding: "1px 7px", borderRadius: 99, background: "#2e1065", color: "#a78bfa" }}>
              {unassignedKeyed.length}
            </span>
            <span style={{ fontSize: 9, color: "#475569", marginLeft: 2 }}>hover a chip + press 1–5 to set priority</span>
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, padding: 12 }}>
            {unassignedKeyed.map(c => <Chip key={c.id} id={c.id} fromWeek={null} />)}
          </div>
        </div>
      )}
      ...
      >
        <div style={{
          ...
          background: "#0f172a", borderBottom: unassignedRaw.length > 0 ? "1px solid #334155" : "none",
        }}>
          ...
          <span style={{
            ...
            background: unassignedRaw.length > 0 ? "#334155" : "#14532d",
            color: unassignedRaw.length > 0 ? "#94a3b8" : "#4ade80",
          }}>
            {unassignedRaw.length > 0 ? unassignedRaw.length : "✓ all assigned"}
          </span>
          {unassignedRaw.length > 0 && (
            <span style={{ fontSize: 9, color: "#334155", marginLeft: 2 }}>hover + press 1–5 to prioritize</span>
          )}
        </div>
        {unassignedRaw.length > 0 && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, padding: 12 }}>
            {unassignedRaw.map(c => <Chip key={c.id} id={c.id} fromWeek={null} />)}
          </div>
        )}
      </div>
        ...
        >
          ...
          >
            ...
            <div style={{ padding: "10px 18px", borderTop: "1px solid #334155", background: "#0f172a", fontSize: 10, color: "#475569", fontFamily: "'DM Mono',monospace" }}>
              Tip: paste into a chat with the Teamwork MCP — ask it to create tasklists under each client&apos;s SEO project using <code style={{ color: "#94a3b8" }}>dueDate</code> and <code style={{ color: "#94a3b8" }}>client</code>.
            </div>
          </div>
        </div>
      )}
