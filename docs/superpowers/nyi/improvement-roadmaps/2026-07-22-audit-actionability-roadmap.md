...
3. **Per-page rescan updates the run IN PLACE** — rescan a single URL (re-render, re-extract on-page SEO, re-verify its links) and update that page's findings within the existing run, with a per-page rescan timestamp shown. A run stops being an immutable snapshot; the page shows "this page is now clean" without a full site scan. **⚠ This deliberately REVERSES the codebase-wide "CrawlRun is an immutable snapshot" semantics** (diffing, sweep classification, characterization tests, archived-blob fallbacks all assume it). Activation requires an explicit full-run-vs-page-rescan fencing design (what a mixed-vintage run means to diff/sweep/score consumers) BEFORE any build — Codex roadmap-review condition, 2026-07-22.
