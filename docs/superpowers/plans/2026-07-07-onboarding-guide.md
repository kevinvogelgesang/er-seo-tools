...
6. **Production topology** [First pass; Senior: read now] — RunCloud VPS + PM2 + nginx; app at `/home/seo/webapps/seo-tools`; DB/uploads/reports under `/home/seo/data/seo-tools/`; logs at `/home/seo/logs/`; deploys = push to GitHub then `~/deploy.sh` (server pulls); Chrome installed on the box; pointer to `08-operations-runbook.md`.
