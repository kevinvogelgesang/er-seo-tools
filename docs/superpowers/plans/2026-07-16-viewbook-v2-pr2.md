...
| 14 | `syncCatalogQuestions` | `service.ts:340` | post-loop | ONE bump `$executeRaw` after the loop iff `added > 0` (documented micro-window; loop shape can't share a txn) |
...
- Produces (all return `Prisma.PrismaPromise<number>` from `prisma.$executeRaw`, composable into array transactions):
  ...
  - `syncVersionBumpWhere(viewbookId: number, predicate: Prisma.Sql)` — scoped bump with an appended `AND (<predicate>)` pre-state guard (mechanisms b/c). Predicate is a `Prisma.sql` fragment; the helper owns the base `UPDATE "Viewbook" SET "syncVersion" = "syncVersion" + 1, "updatedAt" = ${Date.now()} WHERE "id" = ${viewbookId}`.
...
- [ ] **Step 3: Implement** — in each transaction array, add the bump BEFORE the domain statements, predicate mirrored from that path's activity INSERT (rows 1, 2, 4, 5 — copy the exact `WHERE`/`EXISTS` text of the adjacent activity statement into `syncVersionBumpWhere`; keep parameters identical) or from the fenced `updateMany` where (row 3). Do not touch any early-return no-op path.
...
- [ ] **Step 3: Implement per the inventory's mechanism column.** Row 16's `$queryRaw INSERT…RETURNING` single call becomes `$transaction([bump-with-same-guard, $executeRaw INSERT…])` followed by a `findFirst` on `{viewbookId, label, createdBy}` ordered by `id desc` to return the created row (or keep RETURNING by using `$queryRaw` as the LAST statement — array transactions accept `$queryRaw` members; prefer this, it preserves the response shape exactly).
...
- Produces (module `useViewbookSync.ts`):
  ...
  - `useViewbookSync(opts: { url: string; initialVersion: number; intervalMs?: number; onChange: () => void })` — polls `url` (`{v}`) while visible; on `v !== last` OR a pending `requestRefresh`, when registry is idle → calls `onChange` exactly once (coalesced) and clears pending; error backoff doubles to 30 s max; pauses hidden; resumes with an immediate check.
- Consumes: Task 5's endpoints; the public `onChange` is `router.refresh()` (inside `ViewbookSyncClient`), the admin `onChange` is `load()`.

- [ ] **Step 1: Failing hook tests** (jsdom + `vi.useFakeTimers`, fetch mocked): fires `onChange` once when v changes; coalesces poll-change + requestRefresh into ONE onChange; suppresses while an editor is registered active and flushes ONE refresh on release; backs off on fetch error and recovers; does nothing while `document.visibilityState === 'hidden'` (mock) and checks immediately on visibilitychange.
- [ ] **Step 2: Verify failures.**
- [ ] **Step 3: Implement hook + registry** (single module-level `Set<string>` + pending flag + notify target; the hook is mounted once per page — enforce with a module guard that warns on double-mount in dev).
...
- [ ] **Step 5: Wire the page + shell + admin editor** per Files. Admin suspension: reuse the same registry (admin tabs' inputs adopt registration in PR8's inline layer; for now the editor polls + reloads only when `document.activeElement` is not a form control inside the editor container — implement as a small `isFormElementFocused(container)` check, unit-tested).
