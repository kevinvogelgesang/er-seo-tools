...
- `applyCsvRows(rows, clients, schedule): { schedule; clientUpdates: Map<number, Partial<GridClient>>; assignCount; unrecognized: string[] }`
  ...
  normalization, reassignment semantics. (Papa parsing + FileReader stay in
  the page handler; this takes parsed rows.)
...
update + flash, returns `nextPoolChipId(...)` computed against the refs
exactly as today; the keyboard hook assigns the return value to
`setHoveredPoolChipId`.

Status setter and note-modal opener keep their `useCallback` wrappers (the
memoized `Chip` depends on stable handler identities — preserve which handlers
are `useCallback`'d today: `setStatus`, `openNoteModal`; the rest are plain
functions re-created per render, same as now).

`activeLayout`/`layoutName` are **UI state and stay in the page**; the hook's
`applyLayout`/`deleteLayout` take the name as an argument (today's functions
read page state — the page passes it in; identical behavior).
...
- **`LayoutManager.tsx`** — layout `<select>`, delete button, save-as input +
  button. Today `layoutName` and `activeLayout` live in page state but
  nothing outside the layout UI reads either, so **both move into
  `LayoutManager` as local state** (a state-location change invisible to
  behavior). It receives `layouts` plus `saveLayout(name)`/
  `applyLayout(name)`/`deleteLayout(name)` props; toast strings come from
  the hook, so the component stays dumb.
...
Conventions: `@vitest-environment jsdom` for component/hook tests,
`afterEach(cleanup)` (globals:false → no auto-cleanup), local mocks via
`vi.stubGlobal('fetch', …)` + fake timers for the debounce.
...
2. **`useQuarterPlan.test.tsx`** (renderHook) — the B3 plumbing finally under
   ...
   - init: DB plan exists → hydrated state, `canPersist` true, **no PUT
     ...
     PUT calls).
   - init: empty DB + localStorage payload → import POST fired once, toast
     string, hydrate from import response.
...
3. **`Chip.test.tsx`**: status dot cycles in `ALL_STATUSES` order, checkbox
   ...
   `onOpenNote`.
4. Optional (cheap): `NoteModal` 120-char clamp; `LayoutManager` save button
...
- Manual smoke on local dev (`DATABASE_URL="file:./local-dev.db"`): page
  renders, drag works, layout save/apply, gantt toggle, note modal, CSV
  import, save indicator transitions.
...
- **Handler identity changes breaking `memo(Chip)`.** Keep `setStatus` and
  `openNoteModal` as the only `useCallback`s, same as today; Chip re-renders
  are a perf detail, not correctness, but we keep parity anyway.
