...
2. **Practical git/PR workflow** (Codex fix 1): branches, staging, reading `git diff`, commit hygiene, writing a PR description, responding to review comments, conflict basics, and a "before you ask for review" self-checklist. Anchored to this repo's actual conventions (branch naming, PR flow, Kevin as reviewer). Revisited hands-on in Stage 2.
3. JavaScript via MDN's curriculum (variables → functions → objects/arrays → async/promises). Anchor: read a small real file after each unit (e.g. a simple util in `lib/`).
4. TypeScript handbook (types as labels on the JS they just learned). Anchor: read `lib/ada-audit/types.ts` and connect interfaces to JSON they've seen in SEO tools.
5. Node.js basics (what a server process is, npm, `package.json` scripts). Anchor: `package.json` scripts table.
6. **HTTP, APIs, JSON, and browser DevTools** (Codex fix 2): request/response, status codes, JSON, cookies, forms, the Network tab. Anchors: `middleware.ts` (the cookie gate), `lib/api/with-route.ts` (the error envelope), and one simple `app/api/*/route.ts` traced with DevTools open.
7. React fundamentals (components, props, state, effects). Anchor: a small real component, e.g. `components/ThemeToggle.tsx`.
8. Next.js App Router via Next.js Learn (pages, layouts, route handlers, server vs. client components). Anchor: one app route traced end to end.
9. SQL + Prisma basics (tables, relations, migrations). Anchor: read `prisma/schema.prisma` top to bottom with the data-model section of the tour open.
...
Includes a **secrets and env safety** section (Codex fix 3): never commit `.env`; recognize secret shapes (API keys, signed-cookie secrets, service-account JSON) without ever copying values into docs/chats/commits; local vs. production env (prod env lives in PM2's `ecosystem.config.js` on the server); what `DATABASE_URL` points at in each environment; where `lib/auth.ts` gets its secrets. Capability gate: dev server runs, one ADA audit of a test page completes locally, test suite runs, and the junior can state the secrets rules unprompted.

### `03-codebase-tour.md` — the map (reference)
The static structure, written to be skimmable by the senior and readable slowly by the junior. Every section carries a reading-depth label (Codex fix 5): **First pass** (junior reads this paragraph only), **Deep dive later** (junior returns during Stage 1+), **Senior: read now**. Contents:
...
The dynamic behavior, subsystem by subsystem. Same reading-depth labels as 03 (First pass / Deep dive later / Senior: read now):
...
- **Stage 0 — Run it:** local setup complete; click through every tool with the orientation doc open; run one of each audit type **locally, against the designated test targets only** (Codex fix 4 — see exercise safety rules below).
...
Standalone: readable with zero repo context; after reading it (plus a skim of 03/04), a senior can answer "what is this, why is it shaped this way, and what must the junior never merge unreviewed." Sections:
...
- **The big decisions, each as *decision / why / consequences / what would make us revisit*:**
  ...
  6. Auth model (Codex fix 6): signed cookie sessions — Google OAuth as the primary login plus a break-glass password path (`lib/auth.ts`) — with `middleware.ts` owning the gate globally; per-route code never re-checks auth. Document the public-path exceptions (share pages) and token-authed handoff routes in `middleware.ts`, because "new route unexpectedly 401s" is a recurring footgun.
  ...
  8. Lighthouse provider selection (Codex fix 9): the code supports `pagespeed` / `local` / `off`; prod currently runs PSI (`ecosystem.config.js`) to offload CPU from the VPS, with accepted score variance vs. historical local-Lighthouse numbers. Local and off remain supported modes, not dead code.
...
- **Known debt and sharp edges:** `AdaAudit` has no `updatedAt` (job state is the liveness source); the prod-minification bug class (SWC-injected helpers breaking string-injected in-page code; minified class names breaking key lookups); SSRF discipline (Codex fix 8) — never raw-fetch a user-provided URL; all external fetches go through `safeFetch`/the safe-URL helpers in `lib/security/safe-url.ts` (hardened SSRF protection, plus the 999-status hang class); the legacy `SessionPage` fallback pending removal; single-process constraint; memory ceilings; anything else surfaced during writing.
- **Supervising the junior:** review focus checklist (transactions are array-form; new routes use `withRoute`/`parseJsonBody` **but auth never goes in `withRoute` — middleware owns the cookie gate** (Codex fix 7); fenced writes in job code; user-provided URLs only fetched via the safe-URL helpers; `dark:` variants on new UI; tests exist and run against the right DB), the danger zones where they should require your review longest (lib/jobs, lib/findings, schema, recovery, auth/middleware, lib/security), and where the authoritative docs live (CLAUDE.md, this folder, `docs/superpowers/archive/specs/`).
...
- **Exercises are optional and safe.** Concrete safety rails, stated in every exercise section (Codex fix 4): exercises run against the **local dev environment first**; scans/audits only target a designated test-domain list Kevin picks up front (own properties); **no client scans without Kevin's explicit go-ahead**; **no production queue operations of any kind before Stage 4** (avoids accidental client scans, PSI API spend, and queue disruption).
...
## Open questions for Kevin (non-blocking — writable around, but needed before the guide is "done")

- The designated test-domain list for exercises (own properties the junior may scan freely).
- The junior's actual machine (02 ships Mac-first with a WSL2 note either way).
- Production auth posture to state in 07: is the break-glass password login enabled in prod, or Google-only?
