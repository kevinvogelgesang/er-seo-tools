...
- New Prisma model `PillarAnalysis` stores: `sessionId` (FK), `score`, `subscores` (JSON), `hubRecommendation` (JSON with alternates), `pillarTopics` (JSON), `urlVerdicts` (JSON), `dataCompleteness` (0–1), `aiNarrative` (text, nullable, populated later by skill), `narrativeUpdatedAt` (DateTime, nullable), `runnerVersion`, timestamps.
