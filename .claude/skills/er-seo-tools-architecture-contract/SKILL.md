...
4. **No monitoring/alerting** — failures surface only in `$LOG_HOME/` greps and the UI; a dead schedule or repeated dual-write failure is silent until someone looks.
5. **Backup state unverified** — no backup tooling exists in the repo; whether the prod SQLite file is backed up is unknowable from here. Treat `$DATA_HOME/db.sqlite` as precious.
