...
describe('PcInviteSection', () => {
  it('renders the pc-invite title in post-contract', () => {
    const { container } = render(<PcInviteSection meta={defaultMeta()} section={section} data={data()} token="t" />)
  ...
  it('renders the team list with "Invite requested" for invited members, NEVER "Sent" (Codex fix 7)', () => {
    render(<PcInviteSection meta={defaultMeta()} section={section} data={data({ teamMembers: [invitedMember] })} token="t" />)
  ...
  it('renders a distinct status for a not-yet-invited member', () => {
    render(<PcInviteSection meta={defaultMeta()} section={section} data={data({ teamMembers: [uninvitedMember] })} token="t" />)
  ...
  it('shows the ≤15 note and the empty state when no one is invited', () => {
    render(<PcInviteSection meta={defaultMeta()} section={section} data={data()} token="t" />)
  ...
  it('shows the ack action only in post-contract', () => {
    const { rerender } = render(<PcInviteSection meta={defaultMeta()} section={section} data={data()} token="t" />)
    expect(screen.getByRole('button', { name: /looks good/i })).toBeDefined()
    rerender(<PcInviteSection meta={defaultMeta()} section={section} data={data({ stage: 'building' })} token="t" />)
  ...
  it('renders in a carried stage (building) without hard-gating to post-contract', () => {
    const { container } = render(
      <PcInviteSection meta={defaultMeta()} section={section} data={data({ stage: 'building', teamMembers: [invitedMember] })} token="t" />,
    )
    expect(container.textContent).toContain('Invite Your Team')
    expect(screen.getByText('Pat')).toBeDefined()
  })
