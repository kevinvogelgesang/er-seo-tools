...
describe('CollapsibleSection', () => {
  it('defaults to collapsed on a fresh machine: shows heroCollapsed, region hidden+inert', () => {
    ...
    const btn = screen.getByRole('button', { name: 'Expand Brand & Identity' })
  ...
  it('a stored "expanded" value starts expanded', () => {
    ...
    expect(screen.getByRole('button', { name: 'Collapse Brand & Identity' }).getAttribute('aria-expanded')).toBe('true')
  })
  ...
  it('clicking the expanded hero collapses it and persists "collapsed"', () => {
    stored.set(collapseKey(1, 'brand'), 'expanded')
    render(<Harness />)
    const btn = screen.getByRole('button', { name: 'Collapse Brand & Identity' })
    fireEvent.click(btn)
  ...
  it('the whole-hero control is keyboard-activable (native button — Enter/Space handled by the browser)', () => {
    render(<Harness />)
    const btn = screen.getByRole('button', { name: 'Expand Brand & Identity' })
  ...
  it('previewMode: clicking toggles visuals but NEVER writes localStorage', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Collapse Brand & Identity' }))
    expect(screen.getByTestId('hero-collapsed')).toBeDefined()
    expect(stored.size).toBe(0)

    fireEvent.click(screen.getByRole('button', { name: 'Expand Brand & Identity' }))
    expect(screen.getByTestId('hero-expanded')).toBeDefined()
    expect(stored.size).toBe(0)
  })
})
